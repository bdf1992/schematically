...
- **Patterns** — forms put together, and the palette's second half. A Pattern is a record of its own: one click on any part selects it, one drag moves it, one Delete takes it, and its kind, name and color live in one bar, the way a Component's do. Dropping one runs the same creation and hosting paths a gesture runs, so its parts are ordinary forms; OPEN works on them one at a time and RELEASE leaves them loose. A 0D Point is an attachment. A 1D Path carries between its two ends. A 2D Plane bounds a region that hosts Points on its boundary and Components in its interior. Each has a minimal default record; the preset lives in `05-data-core.js`.
