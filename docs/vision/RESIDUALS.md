...
| D2 | Report the engine's `documentHash` finding (E1) | done 2026-09-26: https://github.com/bdf1992/schematically/issues/43#issuecomment-5842516725 (the engine owners already track it as #50; the comment adds a reproduction at `6a39efd` and the layout question, E2) |
...
| E5 | Run surfaces: slice 1c landed them on the state space branch (`6a39efd`: `run.settle`, `state.query`, browser / HTTP / MCP), after this branch merged it at 1b. Save Run / Open Run are slice 2 | merge the state space branch again (13 commits since `32bbacc`); then flipping an input from the canvas, and the export's in-page run, can use the engine's own surfaces |
