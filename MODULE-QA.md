...
| `57-state-view.js` | 119 | Medium | **GREEN** | state space records on the canvas | `state_view_qa` (browser) against the engine's records, `logic_state_export_qa`, mutation | a trace made from a file cannot be shown on it in the editor until the engine's document hash ignores editor defaults |
