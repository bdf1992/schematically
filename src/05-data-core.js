...
(function(root,factory){
  ...
  // A sparse record (a hand-written file, a compact save) takes the same template
  // preset makeComponent applies through the API, and only where the record supplies
  // nothing: authored fields always win.
  function applyTemplatePreset(component){
    const preset=templatePreset(component.symbolId)||{};
    if(!isObject(component.config))component.config={};
    if(!isObject(component.form)&&isObject(preset.form))component.form=clone(preset.form);
    if(!isObject(component.config.presentation)&&isObject(preset.presentation))component.config.presentation=clone(preset.presentation);
    if(!['source','relay','passive'].includes(component.config.signalMode)&&preset.signalMode)component.config.signalMode=preset.signalMode;
    if(!['standard','none'].includes(component.config.attachmentDefaults)&&preset.attachmentDefaults==='none')component.config.attachmentDefaults='none';
    return component;
  }
  ...
  function makeComponent(doc,value={}){
    ...
    const seed=applyTemplatePreset({symbolId,form:isObject(value.form)?clone(value.form):null,config:isObject(value.config)?clone(value.config):{}});
    const id=cleanString(value.id,nextId(doc.components,'c'));
    const canvasId=cleanString(value.canvasId,GLOBAL_CANVAS_ID);
    const form=normalizeComponentForm(seed.form,value.canvas);
    const config={
      label:cleanString(value.config?.label||value.label,''),
      colorSlot:Math.max(0,Math.trunc(num(value.config?.colorSlot,0))),
      signalMode:['source','relay','passive'].includes(seed.config.signalMode)?seed.config.signalMode:'source',
      presentation:isObject(seed.config.presentation)?clone(seed.config.presentation):{},
      ports:isObject(value.config?.ports)?clone(value.config.ports):{}
    };
    if(['standard','none'].includes(seed.config.attachmentDefaults))config.attachmentDefaults=seed.config.attachmentDefaults;
