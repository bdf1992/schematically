...
(function(root,factory){
  ...
  // The document's signal model is behaviour too. A Component with no bound definition that declares
  // a signal (a gate glyph's combine, a lever, a config.signal) is read through the graph core's own
  // signalConfig: a derived binary card becomes a truth_table@1 device over its in and out ports, an
  // asserted one drives its level onto its out ports at power-on. One model, one engine.
  const BINARY_COMBINE={and:v=>v.every(Boolean),min:v=>v.every(Boolean),or:v=>v.some(Boolean),max:v=>v.some(Boolean),
    buffer:v=>v.some(Boolean),not:v=>!v.some(Boolean),nor:v=>!v.some(Boolean),nand:v=>!v.every(Boolean),xor:v=>v.filter(Boolean).length%2===1};
  function combineTable(combine,n,m){
    const rows=[];
    for(let k=0;k<2**n;k++){const bits=[...Array(n)].map((_,i)=>(k>>(n-1-i))&1);const out=BINARY_COMBINE[combine](bits.map(Boolean))?1:0;rows.push([...bits,...Array(m).fill(out)])}
    return rows;
  }
  function impliedDevices(d){
    const g=typeof globalThis!=='undefined'?globalThis:{},G=g.SovSchematicGraph,N=g.SovSchematicNotation,out={definitions:{},bind:{},sources:[]};
    ...
    for(const c of d.components){
      ...
      if(sig.kind!=='binary'||sig.clock)continue;
      ...
      if(sig.mode==='derived'){
        if(!BINARY_COMBINE[sig.combine]||!ins.length||ins.length>8||!outs.length||outs.length>8)continue;
        const ref=`implied.${sig.combine}(${ins.join(',')})>(${outs.join(',')})@1`;
        if(!out.definitions[ref])out.definitions[ref]={id:ref.slice(0,-2),version:1,pattern:'truth_table@1',delay:0,
          parameters:{inputs:ins,outputs:outs,table:combineTable(sig.combine,ins.length,outs.length)}};
        out.bind[c.id]=ref;
  ...
  function topology(d,definitions,bind={},tickMs=1){
    ...
    for(const wire of d.wires){
      ...
      wires.push({id:wire.id,a:{entity:ends.a.entity,point:ends.a.point},b:{entity:ends.b.entity,point:ends.b.point},delay:wireDelay(config,tickMs),
  ...
  function startRun(options){
    ...
    const run={
      ...
      pending:[...inputs,...sources].sort((x,y)=>cmpList([x.at,x.entity,x.point,x.channel],[y.at,y.entity,y.point,y.channel])).map(x=>({kind:'input',...x}))
    };
