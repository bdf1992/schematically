...
(function(root,factory){
  ...
  function nextTick(run){
    ...
    for(const q of Object.values(run.queues))if(q.buf.length>q.head&&(t===null||q.next<t))t=q.next;
    return t;
  }
  function queuedCount(run){return run.pending.length+Object.values(run.queues).reduce((n,q)=>n+(q.buf.length-q.head),0)}
  // A queue merge's buffer: items are only ever appended (`buf`, append-only) or delivered by
  // advancing `head`, so the live queue (`buf.length-head`) never needs shifting or copying. `items`
  // is the plain array external readers (settle's own bookkeeping aside, tests, traces) still see:
  // the live items in order, materialized only when actually read, never on the hot per-tick path.
  function makeQueue(next){
    const q={next};
    Object.defineProperty(q,'buf',{value:[],writable:true,enumerable:false,configurable:true});
    Object.defineProperty(q,'head',{value:0,writable:true,enumerable:false,configurable:true});
    Object.defineProperty(q,'items',{enumerable:true,configurable:true,get(){return this.buf.slice(this.head)}});
    return q;
  }
  // First access of a queue merge's key within a tick records what undoing it needs (never a copy
  // of its items): whether the key existed, and if so its `next`, `head` and buffer length before
  // this tick touches it. Every push, delivery and `next` update after this within the same tick
  // reuses the same queue object, so one record per key per tick is enough.
  function ensureQueue(run,ctx,key){
    let q=run.queues[key];
    if(q){
      if(!ctx.queueUndo.has(key))ctx.queueUndo.set(key,{existed:true,queue:q,next:q.next,head:q.head,bufLength:q.buf.length});
      return q;
    }
    q=makeQueue(ctx.t+1);
  ...
  // entirely; one it found already there has its buffer truncated back to its length before the
  ...
  function undoQueues(run,log){
    for(const [key,entry] of log){
      ...
      const q=entry.queue;
      q.buf.length=entry.bufLength;q.head=entry.head;q.next=entry.next;
      run.queues[key]=q;
    }
  }
  ...
  function updatePort(run,ctx,g){
    ...
    if(win){
      ...
      if(orderFree&&g.arrivals.length){
        ...
        if(g.arrivals.length){const ordered=g.arrivals.length>1?mergeOrder(run,ctx,g,merge):g.arrivals;if(!ordered)return;for(const a of ordered)queue.buf.push({value:a.value,wire:a.wire,phase:a.phase,from:a.from})}
        if(queue.buf.length>queue.head){const item=queue.buf[queue.head++];win={value:item.value,observer:'engine:merge@1',rule:'merge@1',inputs:[item.from]};if(!arrived.includes(item.wire))arrived.push(item.wire)}
  ...
  function processTick(run,t,ctx){
    ...
    for(const [key,q] of Object.entries(run.queues))if(q.buf.length>q.head&&q.next===t){const [entity,point,channel]=JSON.parse(key);group(entity,point,channel)}
    ...
    for(const [key,entry] of ctx.queueUndo){
      const q=entry.queue,from=entry.existed?entry.bufLength:0;
      for(let i=from;i<q.buf.length;i++){const item=q.buf[i];if(typeof item.from==='string'&&item.from.startsWith(TMP))item.from=map.get(item.from)}
      if(q.buf.length===q.head)delete run.queues[key];
    }
  ...
  function settle(run){
    ...
    const syncQueues=()=>{
      ...
      for(const key of Object.keys(run.queues)){
        // Read the engine's own buffer (`buf`/`head`) by index, not the `items` getter: this runs
        // every tick, and materializing a queue's live items here would copy them just as step()
        // must not.
        const q=run.queues[key],buf=q.buf,qh=q.head,liveLen=buf.length-qh,at=i=>buf[qh+i];
