...
(function(root,factory){
  ...
  const FORMS=['binary','continuous','categorical','message'];
  const TIME_MODES=['observed','predicted'];
  const PERTURBATIONS=['none','disturbed','created'];
  const RECORD_KEYS=['format','id','subject','vantage','reference','observable','kind','form','value','time','certainty','observer','provenance','perturbation','principal','hop'];
  ...
  function messageRecord(run,ctx,{entity,point,kind='derived',observer,rule,inputs,message,principal,hop}){
    const id=`${TMP}${ctx.tmp++}`;
  ...
  // it; otherwise it leaves by every carried leg of the component but `via` whose Wire accepts its
  // channel, one child per leg (<id>.<n>, n in leg order); with no such leg it is delivered, unless
  // legs exist and all refuse its channel, when it is refused, never dropped.
  function messageAt(run,ctx,entity,point,message,principal,via,from){
    const component=run.components[entity]||{};
    const end=(event,reason)=>messageRecord(run,ctx,{entity,point,observer:'engine:message',rule:event,inputs:[from],message,principal,hop:reason?{event,reason}:{event}});
    ...
    const forwarded=messageRecord(run,ctx,{entity,point,observer:'engine:fanout',rule:'fanout',inputs:[from],message,principal,hop:{event:'forwarded'}});
    open.forEach((leg,n)=>{
      const child={...copy(message),id:`${message.id}.${n}`,parent:message.id};
      const sent=messageRecord(run,ctx,{entity,point:leg.point,observer:'engine:fanout',rule:'fanout',inputs:[forwarded],message:child,principal,hop:{event:'sent',wire:leg.wire,to:leg.to.entity}});
