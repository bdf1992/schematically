...
function componentConfig(n){
  ...
  // The same limits as the data core and the layout core, so a file never declares a size the
  // screen silently refuses (previously capped at 520×420).
  presentation.size.w=Math.max(80,Math.min(4096,Number(presentation.size.w)||112));
  presentation.size.h=Math.max(64,Math.min(4096,Number(presentation.size.h)||84));
