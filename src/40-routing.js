...
function routePoints(A,B,aSide='out',bSide='in',sourceId=null,targetId=null,laneSeed=0,occupied=[],wireId=null){
  ...
  // With both ends free, nothing dictates an approach direction at either end and there is no
  // boundary to leave, so the carrier is the segment between its two 0D points - the same thing
  // a 1D Form's body is between its own two boundary points. Orthogonal routing exists to meet
  // bound attachment points on their normals; with none to meet, it only invents a bend, which
  // is what made moving a free end look like it reset the line's direction.
  if(!sourceNode&&!targetNode){
    const straight=normalizePoints([A,B]);
    return {points:straight,core:straight,score:0,signature:'free-segment',anchor:routeAnchor(straight),obstacles:[]};
  }
...
function stableRouteForWire(index,w,A,B,occupied=[]){
  ...
  // A free-to-free segment has no interior channel to preserve, and a cached route from when an
  // end was still bound must not survive the unbinding.
  if(candidate.signature==='free-segment'){routeCache.delete(index);return candidate.points}
