...
`logic/` holds circuits for `scripts/logic_sov.py`, written by `scripts/build_logic_examples.py`: every gate in the pack (`gates.sov`), a half adder, a full adder built from two half adders, 4- and 8-bit adders built from those, a 4-bit adder/subtractor, XOR from NANDs, a multiplexer, and an SR latch. See `docs/vision/LOGIC-GATES.md`.
